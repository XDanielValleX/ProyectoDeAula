    ...
    try {
        ...
        while (rs.next()) {
            Object[] fila = {
                ...
                rs.getString("Grupo"),
                rs.getDouble("Nota") // Incluimos las notas
            };
            modelo.addRow(fila);
        }
...
private void actualizarNota(String codigoEstudiante, double nuevaNota) {
    try {
        Connection conexion = ConexionBD.conectar();
        PreparedStatement pst = conexion.prepareStatement("UPDATE estudiantes SET Nota = ? WHERE Codigo_Estudiante = ?");
        pst.setDouble(1, nuevaNota);
        pst.setString(2, codigoEstudiante);

        int filasAfectadas = pst.executeUpdate();

        if (filasAfectadas > 0) {
            cargarDatos(); // Actualizar la tabla después de modificar la nota
